<?php
    ...
    $_QUERY_SQL = "SELECT SUBSTRING(persona_dni, 1, 9) AS SDNI, SUBSTRING(persona_dni, 6, 4) AS bYear FROM persona WHERE SUBSTRING(persona_dni, 6, 4) >= 1970 GROUP BY SDNI LIMIT 10";
    $queryPersona = $connection->query($_QUERY_SQL);
    $total = 0;
    ...
    $_QUERY_SQL = "SELECT * FROM source_name WHERE full_name <> '';";
    $queryPersona = $connection->query($_QUERY_SQL);
    while($persona = mysqli_fetch_array($queryPersona))
    {
        for($i = 1; $i <= 100; $i++)
        {
            $dni = $persona["SDNI"].'-'.str_pad(rand (100, 9999), 5, '0', STR_PAD_LEFT);
            ...
            $birthMonth = str_pad($birthMonth, 2 , '0' , STR_PAD_LEFT );
            $birthDay = str_pad($birthDay, 2 , '0' , STR_PAD_LEFT );
            $fechaNacimiento = $persona["bYear"].'-'.$birthMonth.'-'.$birthDay;


            $Q = array("NUM"=>$i, "dni"=>$dni, "fecha_nacimiento"=>$fechaNacimiento);
            array_push($data, $Q);
            $total = $total + 1;
        }
    }
